...
/**
 * Helper to extract raw text content from children
 * (used to detect LTR/RTL, etc.)
 */
...
export default function MarkdownRenderer({
	...
	// For controlling "expand/collapse" in code blocks
	const [expanded, setExpanded] = useState(false);

	// Create a single object for all custom Markdown components
	const MarkdownComponents = {
		img: ({ src, alt }) => {
			// Check if this is an image generated by AI
		...
		/**
		 * CAREFUL: You have a second 'p' definition near the bottom
		 * that might override this. Combine them if you only need one.
		 */
		p: ({ children }) => {
		...
		/**
		 * This second `p` might override the first one if you keep it.
		 *
		 * Possibly remove or merge with the p above.
		 * I'm leaving it here with the math check as an example of conflict.
		 */
		p: ({ children }) => {
			const text = extractTextContent(children);
			const rtl = isRTL(text);
			...
			// If the content includes math expressions:
			if (text.includes('\\sum') || text.includes('\\int')) {
				return (
